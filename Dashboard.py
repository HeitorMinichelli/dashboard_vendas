...
vendas_estados.rename(columns={'Preço': 'Qtd Vendas'}, inplace=True)
...
vendas_mensal.rename(columns={'Preço': 'Qtd Vendas'}, inplace=True)

vendas_categoria = dados.groupby('Categoria do Produto')[['Preço']].count()
vendas_categoria.rename(columns={'Preço':'Qtd Vendas'}, inplace=True)
...
with aba2:
    ...
    with col1:
        st.metric('Receita', formata_numero(dados['Preço'].sum(), 'R$'))
        fig_vendas_estados = px.scatter_geo(vendas_estados,
                                           lat = 'lat', lon = 'lon',
                                           scope = 'south america',
                                           size = 'Qtd Vendas',
                                           template = 'seaborn',
        ...
        fig_vendas_5estados = px.bar(vendas_estados.head(5), x = 'Qtd Vendas', y = 'Local da compra',
                                   text_auto=True,
                                   title= 'Qtd de Vendas dos 5 principais estados')
        fig_vendas_5estados.update_layout(yaxis={'autorange':'reversed'})
        st.plotly_chart(fig_vendas_5estados)

    with col2:
        st.metric('Quantidade de vendas', formata_numero(dados.shape[0]))
        fig_vendas_mensal = px.line(vendas_mensal, x = 'Mês', y = 'Qtd Vendas',
                                    markers=True, range_y = (0, vendas_mensal.max()),
                                    color = 'Ano', line_dash='Ano',
                                    title= 'Qtd de Vendas p/ Mês')
        st.plotly_chart(fig_vendas_mensal)
        fig_vendas_categoria = px.bar(vendas_categoria, x = vendas_categoria.index, y = 'Qtd Vendas',
                                      text_auto=True,
                                      title = 'Qtd de Vendas p/ Categoria')
